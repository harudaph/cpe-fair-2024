...
type BadgeProps = SpanProps & VariantProps<typeof badgeVariants> & { asChild?: boolean };

/**
 * Badge
 * - Polymorphic: use `asChild` to render as a child element via Radix Slot.
 * - Fully typed: no `any`.
 */
export function Badge({ className, variant, asChild = false, ...props }: BadgeProps) {
  const Comp: React.ElementType = asChild ? Slot : "span";

  // Use span props as the baseline for spreading — this keeps typing strict without `any`.
  const spanProps = props as SpanProps;

  return <Comp data-slot="badge" className={cn(badgeVariants({ variant }), className)} {...spanProps} />;
}
